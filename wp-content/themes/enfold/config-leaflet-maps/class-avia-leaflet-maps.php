...
if( ! class_exists( 'Avia_Leaflet_Maps', false ) )
{
	class Avia_Leaflet_Maps
	{
		...
		public function handler_wp_register_scripts()
		{
			...
			wp_register_script( 'avia_leaflet_maps_backend_script', "$template_url/config-leaflet-maps/js/avia-leaflet-maps{$min_js}.js", array( 'jquery' ), $vn, true );
		...
		{
			return class_exists( 'Leaflet_Map', false ) && $this->is_enabled();
		}
		...
		public function get_maps_html_from_alb_shortode( array $sc_atts )
		{
			...
			foreach( $sc_atts['markers'] as $key => $marker_item )
			{
