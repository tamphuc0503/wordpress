...
if( ! class_exists( 'AviaHelper', false ) )
{
	...
	class AviaHelper
	{
		...
		/**
		 * Returns the HTML title attribute for links
		 *
		 * @since 5.6.4
		 ...
		 * @return string
		 */
		static public function get_link_title_attr_markup( $title_attr, $default = null )
		{
			$title = trim( $title_attr );

			if( $title == '' && is_null( $default ) )
			{
				return '';
			}
		...
		static public function get_url( $link, $post_id = false, $responsive_lightbox = false )
		{
			...
			if( $link[0] == 'manually' )
			{
				 ...
				 * check for e-mail
				 *
				 * @since 5.6.7 added is_email() check and obfuscate filters
				 */
				if( strpos( $link[1], '@' ) !== false && strpos( $link[1], '://' ) === false && is_email( $link[1] ) )
				{
					$e_mail = $link[1];

					/**
					 * Filter to enable this feature
					 *
					 * @since 5.6.7
					 * @param boolean $obfuscate_email_link
					 * @param string $e_mail
					 * @return boolean
					 */
					if( false !== apply_filters( 'avf_obfuscate_email_link', false, $e_mail ) )
					{
						/**
						 * Filter to select hex encoding
						 *
						 * @since 5.6.7
						 * @param int $obfuscate_email_link
						 * @param string $e_mail
						 * @return int						0 | 1
						 */
						$hex_encoding = apply_filters( 'avf_obfuscate_email_link_hex_encoding', 1, $e_mail );

						$hex_encoding = $hex_encoding === 1 ? 1 : 0;

						$e_mail = antispambot( $e_mail, $hex_encoding );
					}

					return 'mailto:' . $e_mail;
				}
...
//    					'\.+$'					=> '',
		...
		/**
		 * Returns if an element is empty. Is intendend to use for objects,
		 * but all types are checked with empty().
		 *
		 * @since 5.3
		 * @param mixed $var
		 * @return boolean
		...
		static public function is_empty( $var )
		{
			...
			//	try to iterate over properties
			foreach( $var as $prop )
			{
				return false;
			}
