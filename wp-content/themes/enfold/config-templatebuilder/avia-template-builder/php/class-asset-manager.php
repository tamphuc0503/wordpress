...
if ( ! class_exists( 'aviaAssetManager', false ) )
{
	class aviaAssetManager extends aviaBuilder\base\object_properties
	{
		/**
		 ...
		 * @since 5.3				removed using .min.* files as this breaks relative urls to background images e.g. in woocommerce-mod.css
		 * @param string $path
		 * @param string $file_type
			...
			if( is_array( $data['remove'] ) )
			{
				...
				foreach( $data['remove'] as $key => $remove )
				{
					if( $remove['path'] != '' )
					{
						if( ! $remove['print'] )
						{
							if( isset( $stored_assets[ $key ] ) && isset( $stored_assets[ $key ]['file_content'] ) )
							{
