...
if ( ! class_exists( 'avia_product_slider', false ) )
{
	class avia_product_slider extends \aviaBuilder\base\aviaSubItemQueryBase
	{
		...
		static public function default_args( array $sc_defaults = array() )
		{
			$default = array(
						...
						'link'					=> 'product_cat',		//	added 5.4.1		taxonomy,term_id,....
						'post_type'				=> 'product',
		...
		public function query_entries( $params = array() )
		{
			...
			/**
			 * Added extended selectbox -> extract categories
			 *
			 * @since 5.4.1
			 */
			if( $params['select_type'] == 'extended' )
			{
				$ext_link = explode( ',', $params['link'] );
				...
				{
					$params['taxonomy'] = $ext_link[0];
					unset( $ext_link[0] );
					$params['categories'] = implode( ',', $ext_link );	//	we need to renumber index later
				}
			}
			...
			if( ! empty( $params['categories'] ) )
			{
				...
				$terms = ! is_array( $params['categories'] ) ? explode( ',', $params['categories'] ) : $params['categories'];
			}
