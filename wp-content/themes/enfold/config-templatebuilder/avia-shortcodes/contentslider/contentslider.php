...
if ( ! class_exists( 'avia_sc_content_slider', false ) )
{
...
if ( ! class_exists( 'avia_content_slider', false ) )
{
	class avia_content_slider extends \aviaBuilder\base\aviaSubItemQueryBase
	{
		...
		public function html()
		{
			...
			foreach( $content as $key => $value )
			{
				$link = '';
				$linktarget = '';
				$title_attr_val = ! empty( $value['attr']['title_attr'] ) ? $value['attr']['title_attr'] : $value['attr']['title'];
				...
				$output .= ( ! empty( $link ) && ! empty( $title ) ) ? "<a href='{$link}' $blank title='" . esc_attr( $title_attr_val ) . "'>{$title}</a>" : $title;
