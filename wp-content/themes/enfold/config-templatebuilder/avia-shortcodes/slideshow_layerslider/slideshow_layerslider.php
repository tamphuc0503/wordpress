...
if( ! class_exists( 'avia_sc_layerslider', false ) )
{
	class avia_sc_layerslider extends aviaShortcodeTemplate
	{
		...
		protected function extra_assets()
		{
			...
			//load css
			wp_enqueue_style( 'avia-module-slideshow-ls', AviaBuilder::$path['pluginUrlRoot'] . "avia-shortcodes/slideshow_layerslider/slideshow_layerslider{$min_css}.css", array('avia-layout'), $ver );

				//load js
			wp_enqueue_script( 'avia-module-slideshow-ls', AviaBuilder::$path['pluginUrlRoot'] . "avia-shortcodes/slideshow_layerslider/slideshow_layerslider{$min_js}.js", array('avia-shortcodes'), $ver, true );
		}
		...
		public function shortcode_handler( $atts, $content = '', $shortcodename = '', $meta = array() )
		{
			...
			if( class_exists( 'LS_Shortcode', false ) && method_exists( 'LS_Shortcode', 'handleShortcode' ) ) //fix for search results page - only works with latest LayerSlider versions
			{
