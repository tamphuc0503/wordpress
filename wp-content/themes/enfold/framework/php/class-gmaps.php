...
if( ! class_exists( 'av_google_maps', false ) )
{
	class av_google_maps extends aviaFramework\base\object_properties
	{
			//	maintain URL and version number here for all objects using this element
		const API_URL = 'https://maps.googleapis.com/maps/api/js';
		const API_VERSION = '3.52';
		...
		protected function __construct( $key = '' )
		{
			...
			add_filter( 'avf_custom_body_classes', array( $this, 'handler_avf_custom_body_classes' ), 10, 1 );
		...
		public function handler_wp_register_scripts()
		{
			...
			$api_url = av_google_maps::api_url( $api_key ) . '&callback=aviaOnGoogleMapsLoaded';

			wp_register_script( 'avia-google-maps-api', $api_url, array( 'jquery', 'avia_google_maps_api_script' ), null, true );
		 ...
		 * @return string
		 */
		public function handler_avf_custom_body_classes( $body_class )
		{
			/**
			 * @since 5.4
			 * @param boolean $no_google_fonts
			 * @return boolean						true to suppress loading of Google Fonts
			 */
			if( true === apply_filters( 'avf_gmaps_no_google_fonts', false ) )
			{
				$body_class .= ' av-gmaps-no-google-fonts';
			}
