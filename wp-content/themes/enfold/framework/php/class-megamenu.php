...
if( ! class_exists( 'avia_megamenu', false ) )
{
	/**
	...
	class avia_megamenu
	{
		 ...
		 * @since ????
		 */
		public function avia_menu_header()
		{
			if( basename( $_SERVER['PHP_SELF'] ) == 'nav-menus.php' )
			{
				...
				wp_enqueue_style( 'avia_admin', AVIA_CSS_URL . "avia_admin{$min_css}.css", array(), $vn, 'all' );
				wp_enqueue_script( 'avia_mega_menu', AVIA_JS_URL . "avia_mega_menu{$min_js}.js", array( 'jquery', 'jquery-ui-sortable' ), $vn, true );
			}
		}
...
if( ! class_exists( 'avia_walker', false ) )
{
	/**
...
if( ! class_exists( 'avia_backend_walker', false ) )
{
/**
	...
	class avia_backend_walker extends Walker_Nav_Menu
	{
		...
		public function start_el( &$output, $item, $depth = 0, $args = array(), $current_object_id = 0 )
		{
			...
			if( ! empty( $item->_invalid ) )
			{
				...
				$title = sprintf( __( '%s (Invalid)', 'avia_framework' ), $item->title );
			}
			else if( isset( $item->post_status ) && 'draft' == $item->post_status )
			{
				$classes[] = 'pending';
				/* translators: %s: title of menu item in draft status */
				$title = sprintf( __( '%s (Pending)', 'avia_framework' ), $item->title );
			}
				...
				<dl class="menu-item-bar">
					<dt class="menu-item-handle">
						...
						<span class="item-controls">
							...
							?>"><?php _e( 'Edit Menu Item', 'avia_framework', 'avia_framework' ); ?></a>
						</span>
					</dt>
				</dl>
				...
				<div class="menu-item-settings wp-clearfix" id="menu-item-settings-<?php echo $item_id; ?>">
					...
					<p class="field-description description description-wide">
						<label for="edit-menu-item-description-<?php echo $item_id; ?>">
							<?php _e( 'Description', 'avia_framework' ); ?><br />
